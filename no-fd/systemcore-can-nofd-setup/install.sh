#!/bin/bash
# Install the DIY SystemCore CAN bring-up (Pi 5 + Waveshare 2-CH CAN HAT) onto an
...
#   sudo ./install.sh --no-config          # leave config.txt alone (you pasted ../config_no_fd.txt yourself)
#   sudo ./install.sh --int0 22 --int1 24  # HAT with re-soldered INT pads
#
# config.txt: only the stock lines that collide with the HAT are commented out ("#diy-can: ")
# and the "# >>> diy-can ... # <<< diy-can" block from ../config_no_fd.txt is written; everything
# else in the file (other add-ons' blocks included) is left alone.
set -e

CONFIG_NAME=config_no_fd.txt   # looked for next to this script, then one directory up
BLOCK_BEGIN='# >>> diy-can'
BLOCK_END='# <<< diy-can'
