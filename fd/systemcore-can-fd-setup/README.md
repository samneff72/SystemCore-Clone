...
DIY substitute that drives CTRE Phoenix 6 devices over the **Waveshare 2-CH CAN FD HAT**
...
> This FD variant was updated together with the no-FD one but has not been run on an FD
> HAT yet. Please report results.
...
1. Edits `config.txt` in place on both boot slots (`/dev/mmcblk0p2` and `p3`): the stock
   ...
   `# >>> diy-can` and `# <<< diy-can` from `../config_with_fd.txt` is added. Nothing else in
   the file is touched.
2. Installs a udev rule that names the two MCP2518FD buses by **stable SPI path**:
   - `spi0.0` → `can_s0`  (addressed in code as `CANBus.systemcore(0)`)
   - `spi1.0` → `can_s1`  (`CANBus.systemcore(1)`)
3. Points the stock `limelight_canbusprocess.service` at `diy-can-setup.sh` (systemd
   ...
   The stock unit has to be overridden rather than supplemented: since image 13 it
   configures all five buses in one `&&` chain, fails on the vcan dummy `can_s2`, and is
   restarted every 5 s — bouncing `can_s0`/`can_s1` each time.
...
| --- | --- | --- | --- |
| 2-CH CAN FD HAT fitted | physical @ 1 Mbit | vcan | `CANBus.systemcore(0/1)` |
| No HAT | vcan | vcan | CANivore: `new CANBus("reefmaster")` |
...
- For physical CAN: the **Waveshare 2-CH CAN FD HAT** in Mode A (CAN_0 on `spi0.0`, INT
  GPIO 25; CAN_1 on `spi1.0`, INT GPIO 24) with proper 120 Ω bus termination. Other
  interrupt pins: `--int0 N --int1 M`. (No HAT? It still works over a CANivore.)
...
Copy the `fd` folder to the Pi and run the installer:
```bash
...
sudo reboot
```
(`--no-config` skips the `config.txt` edit if you pasted `config_with_fd.txt` in by hand.)
...
| --- | --- | --- |
| `CANBus.systemcore(0)` | `can_s0` | HAT CAN_0 — MCP2518FD on `spi0.0` (INT GPIO 25) |
| `CANBus.systemcore(1)` | `can_s1` | HAT CAN_1 — MCP2518FD on `spi1.0` (INT GPIO 24) |
